...
count=$((count + 1))
echo "${count}" > "${COUNTER}"
echo "wifucked-bootcount: boot ${count} of ${THRESHOLD}"

if [[ "${count}" -ge "${THRESHOLD}" ]]; then
    echo "wifucked-bootcount: FACTORY RESET — clearing WAN configuration"
    ...
    rm -f "${COUNTER}"
    logger -t wifucked "factory reset performed via boot count; LAN identity preserved"
    systemctl restart wifucked.service || true
    exit 0
fi
...
(
    sleep "${CLEAR_AFTER}"
    rm -f "${COUNTER}"
    echo "wifucked-bootcount: uptime threshold reached; counter cleared"
) &
