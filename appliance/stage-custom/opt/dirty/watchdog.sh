...
#
# It touches ONLY the `current` symlink, its own sentinels, and dirty.service.
...
log() { echo "[$(date -u +%FT%TZ)] dirty-watchdog: $1"; }
...
CONFIG_FILE="${DIRTY_STATE_DIR:-/var/lib/dirty}/config.json"
...
health_ok() {
    systemctl is-active --quiet dirty.service || return 1
    local body
...
if [[ -z "${REVERT_TO}" || ! -d "${REVERT_REAL}" ]]; then
    log "current version UNHEALTHY (${CUR:-unknown}) but no valid previous slot to roll back to; leaving last-known-good in place (dirty.service Restart=always keeps retrying)"
    exit 1
fi
...
if systemctl restart dirty.service; then
    log "rollback complete; dirty.service restarted on ${REVERT_REAL}"
else
    log "WARNING dirty.service restart failed after rollback; Restart=always will retry on ${REVERT_REAL}"
fi
