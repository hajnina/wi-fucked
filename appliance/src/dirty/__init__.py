"""DIRTY → BALANCED — an autonomous connectivity appliance.
...
#: Fallback only. The real version is baked into /etc/dirty-release at image
#: build time and read via dirty.config.release_info() — the git tag is the
