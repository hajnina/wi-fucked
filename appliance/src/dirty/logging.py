"""Resilient structured logging for the dirty daemon.

Every logger in this application is rooted at ``dirty``. Use :func:`get_logger`,
...
_LOG_FILE_CANDIDATES = ("/var/log/dirty.log", "/tmp/dirty.log")  # noqa: S108
...
def _configure_root() -> logging.Logger:
    ...
    root.setLevel(logging.DEBUG if os.getenv("DIRTY_DEBUG") else logging.INFO)
...
def get_logger(name: str) -> logging.Logger:
    """Return a resilient logger rooted at ``dirty``.

    ``get_logger("allocator")`` yields ``dirty.allocator``. Passing an
