...
from dirty.policy import Thresholds

log = get_logger("config")

DEFAULT_STATE_DIR = Path(os.getenv("DIRTY_STATE_DIR", "/var/lib/dirty"))
RELEASE_FILE = Path(os.getenv("DIRTY_RELEASE_FILE", "/etc/dirty-release"))
...
def release_info() -> dict[str, str]:
    """Read ``/etc/dirty-release``, baked into the image at build time."""
    info: dict[str, str] = {}
    try:
        ...
        info = {"DIRTY_VERSION": "0.0.0-dev", "DIRTY_CHANNEL": "development"}
    return info
