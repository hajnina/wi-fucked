...
from dirty import __version__
from dirty.api import create_app
from dirty.config import load
...
def _install_scenario(daemon: Daemon, name: str) -> None:
    ...
    ``DIRTY_SCENARIO=moving_van`` reproduces the field conditions the product
    exists to survive, so the dashboard can be looked at without a van.
    """
    try:
        from dirty.scenarios import install
...
def main() -> int:
    ...
    scenario = os.getenv("DIRTY_SCENARIO")
    if scenario:
        _install_scenario(daemon, scenario)

    daemon.start()

    loops = threading.Thread(target=daemon.run_forever, name="dirty-loops", daemon=True)
    loops.start()
